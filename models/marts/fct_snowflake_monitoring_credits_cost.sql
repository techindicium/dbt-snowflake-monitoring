    ...
    , Duplicatas AS (
        SELECT
            {{ dbt_utils.generate_surrogate_key(
                ['filtered_fact_table.query_id', 'filtered_fact_table.warehouse_id','filtered_fact_table.database_id', 'filtered_fact_table.schema_id', 'filtered_fact_table.session_id', 'filtered_fact_table.user_name', 'filtered_fact_table.start_time']
                ) }} AS metrics_sk
        ...
        LEFT JOIN dim_database 
            ON filtered_fact_table.database_id = dim_database.database_id
        LEFT JOIN dim_warehouse 
            ON filtered_fact_table.warehouse_id = dim_warehouse.warehouse_id
        LEFT JOIN dim_schema 
            ON filtered_fact_table.schema_id = dim_schema.schema_id
        LEFT JOIN dim_dates 
            ON filtered_fact_table.start_time::date = dim_dates.date
        LEFT JOIN dim_user 
            ON filtered_fact_table.user_name = dim_user.user_name
    )
    SELECT
        metrics_sk
        , user_fk
